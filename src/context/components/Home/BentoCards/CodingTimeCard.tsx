...
import NumberTicker from '@/context/components/ui/NumberTicket'
import Link from 'next/link'
import CodeIcon from '@/context/components/Icons/CodeIcon'
import { VsCode } from '@/context/components/Icons/StackIcons/VS'
import { Terminal } from '@/context/components/Icons/Terminal'
...
async function CodingTimeCard() {
  ...
  return (
    <BentoCard className="group col-span-5 row-span-1 flex h-full flex-col items-start justify-start p-4 font-mono xs:col-span-3 xs:row-span-2">
      <CodeIcon className="size-5 fill-gray-900 dark:fill-stone-600" />
      <Link
        href={'https://wakatime.com/@akshitasrivastava20'}
        target="_blank"
      ...
      >
        ...
        <div>
          <span className="bg-gradient-to-b from-white to-black bg-clip-text text-5xl font-bold text-transparent">
            {hours ? (
              <NumberTicker
                className="bg-gradient-to-b from-white to-black bg-clip-text font-mono text-5xl font-bold text-transparent"
                value={hours}
              />
            ) : (
              '--'
          ...
          <span className="bg-gradient-to-b from-blue-500 from-50% to-[#020122] bg-clip-text font-mono text-2xl font-semibold text-transparent">
            {' '}
            hrs
          </span>
        </div>
        <div className="text-center font-mono font-medium text-emerald-400">
